...
//      FIXME, CAUSES PROBLEMS IN GTA:VCS, possibly others 
        if ((!output2init) && (samplerate != 44100))
        {
            ...
            res = sceAudio_driver_837701CC(OUTPUT_BUFFER_SIZE/4, samplerate, 2);

            if (res >= 0)//sucessfully alloc'd a channel, returns < 0 on resume
            {
		printf("output2init = 1;\n\n");
                output2init = 1;
                music->audio_id = 9;
            }
        }
            ...
            if (!output2init)
		{
			...
			//sceAudioOutputBlocking(music->audio_id, PSP_AUDIO_VOLUME_MAX, OutputBuffer[OutputBuffer_flip] );
		}            
		else
            {
		printf("sceAudio_driver_AC81DE4F\n\n");
                 res = sceAudio_driver_AC81DE4F(PSP_AUDIO_VOLUME_MAX*music->volume/100, OutputBuffer[OutputBuffer_flip]);//sceAudioOutput2OutputBlocking
